...
const CreateChannelModal = ({ isOpen, onClose, onCreateChannel, onJoinChannel }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === 'create' && channelName.trim()) {
      ...
      setChannelName('');
      setIsPrivate(false);
    } else if (mode === 'join' && inviteCode.trim() && onJoinChannel) {
      onJoinChannel(inviteCode);
      setInviteCode('');
    }
  };
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white">
              {mode === 'create' ? 'Create a new channel' : 'Join a private channel'}
            </h3>
          ...
          <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-xl">
            <button type="button" onClick={() => setMode('create')} className={`flex-1 py-1.5 text-sm font-semibold rounded-lg transition-all ${mode === 'create' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-[#76ABAE] shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Create</button>
            <button type="button" onClick={() => setMode('join')} className={`flex-1 py-1.5 text-sm font-semibold rounded-lg transition-all ${mode === 'join' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-[#76ABAE] shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Join via Code</button>
          </div>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4">
            {mode === 'create' ? (
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Invite Code</label>
                <div className="relative">
                  <input
                    type="text"
                    value={inviteCode}
                    ...
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-center tracking-[0.2em] font-mono font-bold"
                  />
                </div>
              </div>
            )}
          ...
          <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-b-3xl flex justify-end gap-3">
            ...
            <button type="submit" disabled={mode === 'create' ? !channelName.trim() : !inviteCode.trim()} className="px-5 py-2.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
